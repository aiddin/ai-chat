...
${this.welcomeSubtitle}` : this.welcomeMessage;
  ...
  saveMessagesToStorage() {
    try {
      ...
      console.warn("Failed to save messages to sessionStorage:", error2);
    }
  }
  loadMessagesFromStorage() {
    try {
      ...
      console.warn("Failed to load messages from sessionStorage:", error2);
    }
    return null;
  }
  clearMessagesFromStorage() {
    try {
      ...
      console.warn("Failed to clear messages from sessionStorage:", error2);
    }
  }
    ...
    if (this.initialMessages && this.initialMessages.length > 0) {
      ...
      if (this.initialQuestionsUrl) {
        try {
          ...
          console.warn("Failed to fetch initial questions:", error2);
          fetchFailed = true;
        }
      }
      if (fetchFailed) {
...
${this.welcomeSubtitle}` : this.welcomeMessage;
   ...
   */
  async fetchQuestionTexts(questions) {
    ...
    const fetchPromises = questions.map(async (q) => {
      try {
        ...
        console.error(`Error fetching question ${q.Id}:`, error2);
        return null;
      }
    });
      ...
      if (suggestedQuestions && suggestedQuestions.length > 0) {
        ...
        if (questionsNeedingText.length > 0) {
          ...
          suggestedQuestions = [
            ...
            // Keep questions that already have text
            ...fetchedQuestions
            // Add newly fetched questions
          ];
        }
      }
      const assistantMessage = {
